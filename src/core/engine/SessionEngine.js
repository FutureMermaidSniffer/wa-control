...
export class SessionEngine {
  ...
  async startLink(accountId, opts = {}) {
    ...
    if (mode === 'pairing') {
      ...
      const result = await this.manager.requestPairingCode(accountId, phone, {
        forceNew: !!opts.forceNew,
        forceDirect: !!opts.forceDirect,
      });
      // Gate may return { code, handshakeStatus, handshakeWaitMs } or a bare string
      const pairingCode = typeof result === 'string' ? result : result?.code;
      return {
        mode: 'pairing',
        accountId,
        phone,
        ...
        ...(typeof result === 'object' && result
          ? {
              ...
              handshakeWaitMs: result.handshakeWaitMs,
              code: result.code,
            }
          : {}),
      };
  ...
  getPairingCodes(opts) {
    return this.manager.getPairingCodes?.(opts) ?? this.manager.getPairingCodes?.() ?? [];
  }

  getPairingDiagnostics() {
    return this.manager.getPairingDiagnostics?.() ?? {};
  }
